...
void test_triangle_translate() {
    ...
    assert(vec_isclose(*(vector_t *) list_get(tri, 1), (vector_t) {0,  -3}));
    assert(vec_isclose(*(vector_t *) list_get(tri, 0), (vector_t) {0,  0}));
    assert(isclose(polygon_area(tri), 6));
...
void test_triangle_rotate() {
    ...
    assert(vec_isclose(*(vector_t *) list_get(tri, 2), (vector_t) {-1,  3}));
    assert(vec_isclose(*(vector_t *) list_get(tri, 1), (vector_t) {2.2, 0.6}));
    assert(vec_isclose(*(vector_t *) list_get(tri, 0), (vector_t) {4,   3}));
    assert(isclose(polygon_area(tri), 6));
...
void test_circ_translate() {
    ...
    for (size_t i = CIRC_NPOINTS - 1; i >= 0; i--) {
        double angle = 2 * M_PI * (CIRC_NPOINTS - 1 - i) / CIRC_NPOINTS;
        assert(vec_isclose(
...
void test_weird_translate() {
    ...
    assert(vec_isclose(*(vector_t *) list_get(w, 4), (vector_t) {-10, -20}));
    assert(vec_isclose(*(vector_t *) list_get(w, 3), (vector_t) {-6,  -19}));
    assert(vec_isclose(*(vector_t *) list_get(w, 2), (vector_t) {-12, -19}));
    assert(vec_isclose(*(vector_t *) list_get(w, 1), (vector_t) {-15, -15}));
    assert(vec_isclose(*(vector_t *) list_get(w, 0), (vector_t) {-11, -28}));
    assert(isclose(polygon_area(w), 23));
